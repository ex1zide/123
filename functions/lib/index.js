...
exports.analyzeContract = exports.upgradePlan = exports.askAI = void 0;
...
const SYSTEM_PROMPT = "Ты — корпоративный ИИ-юрист Республики Казахстан. " +
...
exports.askAI = functions.https.onCall(async (data, context) => {
    // ── 1. Auth Check ──
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Авторизация обязательна для использования ИИ-юриста.");
    }
    const uid = context.auth.uid;
    const question = data === null || data === void 0 ? void 0 : data.question;
    if (!question || typeof question !== "string" || question.trim().length === 0) {
        throw new functions.https.HttpsError("invalid-argument", "Вопрос не может быть пустым.");
    }
    ...
    if (plan === "free" && usedQueries >= maxQueries) {
        throw new functions.https.HttpsError("resource-exhausted", "Лимит бесплатных запросов исчерпан. Перейдите на PRO.");
    }
    // ── 3. Sanitize input ──
...
exports.upgradePlan = functions.https.onCall(async (data, context) => {
    ...
    const uid = context.auth.uid;
    const newPlan = data === null || data === void 0 ? void 0 : data.plan;
...
// ════════════════════════════════════════════════════════════
const CONTRACT_SYSTEM_PROMPT = "Ты — корпоративный юрист Республики Казахстан, специализирующийся на анализе договоров. " +
...
exports.analyzeContract = functions.https.onCall(async (data, context) => {
    // ── 1. Auth Check ──
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Авторизация обязательна.");
    }
    const contractText = data === null || data === void 0 ? void 0 : data.contractText;
    if (!contractText || typeof contractText !== "string" || contractText.trim().length < 10) {
        throw new functions.https.HttpsError("invalid-argument", "Текст договора слишком короткий или отсутствует.");
    }
    ...
    const result = await model.generateContent(`ТЕКСТ ДОГОВОРА:\n\n${sanitizedText}`);
    const response = result.response;
    const aiText = response.text() || "";
